...
/*                              External Functions                           */
/*****************************************************************************/
float J_MSE(float* search, int search_width, float* pattern, int pat_height, int pat_width) {
    ...
    try {
        ...
        for (int r = 0; r < height-height1; r++) {
            for (int c = 0; c < width-width1; c++) {
                ...
                float result = J_MSE(search_coord, width, input_comps1[0].buf, height1, width1);
                if (result < min) {     // Keep track of coordinates of minimum error
                    min = result;
                    pix_y = r;
                    pix_x = c;
                }
